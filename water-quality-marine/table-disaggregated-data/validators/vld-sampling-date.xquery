...
module namespace vldwqlmdismpsdate = 'http://converters.eionet.europa.eu/wise/waterQualityMarine/disaggregatedData/validators/samplingDate';
...
declare function vldwqlmdismpsdate:validate-sampling-date(
...
as element(result)
{
    ...
    let $resultRows := vldwqlmdismpsdate:_validate($columnSamplingDate, $dateStart, $dateEnd, $dataRows, 1, ())
...
declare function vldwqlmdismpsdate:get-data-flow-cycle($dataFlowCycles as element(DataFlows))
as element(DataFlowCycle)
{
    $dataFlowCycles/DataFlow[@RO_ID="14"]/DataFlowCycle[@Identifier="2017"]
};

declare function vldwqlmdismpsdate:_validate(
...
as element(row)*
{
    ...
    (for $row in $dataRows
    return vldwqlmdismpsdate:_validate-data-row($columnSamplingDate, $dateStart, $dateEnd, $row)
    ...
    (:if ($dataRowIndex > count($dataRows)) then
        ...
        let $rowResult := vldwqlmdismpsdate:_validate-data-row($columnSamplingDate, $dateStart, $dateEnd, $dataRow)
        let $newResultRows :=
            if (empty($rowResult)) then
                $resultRows
            else
                ($resultRows, $rowResult)
        return vldwqlmdismpsdate:_validate($columnSamplingDate, $dateStart, $dateEnd, $dataRows, $dataRowIndex + 1, $newResultRows):)
};

declare function vldwqlmdismpsdate:_validate-data-row(
    $columnSamplingDate as element(column), 
    $dateStart as xs:date,
    $dateEnd as xs:date,
...
as element(row)?
{
    ...
    return
        if (empty($samplingDate) or (year-from-date($samplingDate) >= year-from-date($dateStart) and year-from-date($samplingDate) <= year-from-date($dateEnd))) then
            ()
        else
