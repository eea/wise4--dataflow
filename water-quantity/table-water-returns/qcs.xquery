...
module namespace wqnwaterret = 'http://converters.eionet.europa.eu/wise/waterQuantity/waterReturns';
...
declare variable $wqnwaterret:TABLE-ID := "9734";

declare function wqnwaterret:run-checks($sourceUrl as xs:string)
as element(div)
{
    let $dataDoc := doc($sourceUrl)/*:WaterReturns
    let $model := meta:get-table-metadata($wqnwaterret:TABLE-ID)
    ...
    return wqnwaterret:run-checks($dataDoc, $model, $envelope, $vocabularySpatialUnits, $dataFlowCycles, $limitDefinitions)
};

declare function wqnwaterret:run-checks(
    $dataDoc as element()*,
...
as element(div)
{
    let $qcs := wqnwaterret:getQcMetadata($model, $envelope, $dataFlowCycles)
    let $dataRows := data:get-rows($dataDoc)
    let $qcResultsMarkup := 
        <div>
            { wqnwaterret:_run-mandatory-field-qc($qcs/qc[@id="qc1"], $model, $dataRows) }
            { wqnwaterret:_run-duplicate-rows-qc($qcs/qc[@id="qc2"], $model, $dataRows) }
            { wqnwaterret:_run-data-types-qc($qcs/qc[@id="qc3"], $model, $dataRows) }
            { wqnwaterret:_run-codelists-qc($qcs/qc[@id="qc4"], $model, $dataRows) }
            { wqnwaterret:_run-spatial-unit-id-format-qc($qcs/qc[@id="qc5a"], $model, $envelope, $dataRows) }
            { wqnwaterret:_run-spatial-unit-id-reference-qc($qcs/qc[@id="qc5b"], $model, $vocabularySpatialUnits, $dataRows) }
            { wqnwaterret:_run-spatial-unit-id-scheme-qc($qcs/qc[@id="qc6"], $model, $dataRows) }
            { wqnwaterret:_run-time-reference-period-qc($qcs/qc[@id="qc7"], $model, $dataFlowCycles, $dataRows) }
            { wqnwaterret:_run-time-period-volume-sum-qc($qcs/qc[@id="qc8"], $model, $dataRows) }
            { wqnwaterret:_run-observed-volume-limits-qc($qcs/qc[@id="qc9"], $model, $limitDefinitions, $dataRows) }
        </div>
    return 
        <div class="feedbacktext"> 
            ...
            { html:getJavascript() }
            <div>
                { uiutil:build-header-and-menu-markup("WISE SoE - Water Quantity", "Water Returns", $qcs, $qcResultsMarkup) }
                { $qcResultsMarkup }
            </div>
        </div>
};

declare function wqnwaterret:getQcMetadata(
    $model as element(model),
    $envelope as element(envelope),
    $dataFlowCycles as element(DataFlows)
)
...
{
    <qcs>
        { wqnwaterret:_get-mandatory-qc-metadata($model) }
        { wqnwaterret:_get-duplicate-rows-qc-metadata($model) }
        { wqnwaterret:_get-data-types-qc-metadata($model) }
        { wqnwaterret:_get-codelists-qc-metadata($model) }
        { wqnwaterret:_get-spatial-unit-id-format-qc-metadata($envelope) }
        { wqnwaterret:_get-spatial-unit-id-reference-qc-metadata() }
        { wqnwaterret:_get-spatial-unit-id-scheme-qc-metadata() }
        { wqnwaterret:_get-time-reference-period-qc-metadata($dataFlowCycles) }
        { wqnwaterret:_get-time-period-volume-sum-qc-metadata() }
        { wqnwaterret:_get-observed-volume-limits-qc-metadata() }
    </qcs>
};

declare function wqnwaterret:_get-mandatory-qc-metadata($model as element(model))
as element(qc)
{
...
declare function wqnwaterret:_get-duplicate-rows-qc-metadata($model as element(model))
as element(qc)
{
...
declare function wqnwaterret:_get-data-types-qc-metadata($model as element(model))
as element(qc)
{
    <qc id="qc3">
...
declare function wqnwaterret:_get-codelists-qc-metadata($model as element(model))
as element(qc)
{
    ...
    return
        <qc id="qc4">
            ...
            <codelistUrls>
                ...
                <url columnName="observedProperty" value="http://dd.eionet.europa.eu/fixedvalues/elem/84134" />
                <url columnName="resultObservationStatus" value="http://dd.eionet.europa.eu/fixedvalues/elem/77669" />
            </codelistUrls>
        </qc>
};

declare function wqnwaterret:_get-spatial-unit-id-format-qc-metadata($envelope as element(envelope))
as element(qc)
{
...
declare function wqnwaterret:_get-spatial-unit-id-reference-qc-metadata()
as element(qc)
{
    <qc id="qc5b">
...
declare function wqnwaterret:_get-spatial-unit-id-scheme-qc-metadata()
as element(qc)
{
    <qc id="qc6">
...
declare function wqnwaterret:_get-time-reference-period-qc-metadata($dataFlowCycles as element(DataFlows))
as element(qc)
{
...
declare function wqnwaterret:_get-time-period-volume-sum-qc-metadata()
as element(qc)
{
    <qc id="qc8">
...
declare function wqnwaterret:_get-observed-volume-limits-qc-metadata()
as element(qc)
{
    <qc id="qc9">
...
declare function wqnwaterret:_run-mandatory-field-qc($qc as element(qc), $model as element(model), $dataRows as element(dataRow)*)
as element(div)
{
    let $mandatoryColumns := meta:get-mandatory-columns($model)
...
declare function wqnwaterret:_run-duplicate-rows-qc($qc as element(qc), $model as element(model), $dataRows as element(dataRow)*)
as element(div)
{
    let $keyColumns := meta:get-primary-key-columns($model)
...
declare function wqnwaterret:_run-data-types-qc($qc as element(qc), $model as element(model), $dataRows as element(dataRow)*)
as element(div)
{
...
declare function wqnwaterret:_run-codelists-qc($qc as element(qc), $model as element(model), $dataRows as element(dataRow)*)
as element(div)
{
    let $columnsToDisplay := $model/columns/column
...
declare function wqnwaterret:_run-spatial-unit-id-format-qc($qc as element(qc), $model as element(model), $envelope as element(envelope), $dataRows as element(dataRow)*)
as element(div)
{
...
declare function wqnwaterret:_run-spatial-unit-id-reference-qc(
    $qc as element(qc), 
    $model as element(model), 
    $vocabularySpatialUnits as element(), 
...
declare function wqnwaterret:_run-spatial-unit-id-scheme-qc(
    $qc as element(qc),
    $model as element(model),
    $dataRows as element(dataRow)*
)
...
declare function wqnwaterret:_run-time-reference-period-qc(
    $qc as element(qc), 
    $model as element(model),
    $dataFlowCycles as element(DataFlows),
    $dataRows as element(dataRow)*
...
declare function wqnwaterret:_run-time-period-volume-sum-qc($qc as element(qc), $model as element(model), $dataRows as element(dataRow)*)
as element(div)
{
...
declare function wqnwaterret:_run-observed-volume-limits-qc(
    $qc as element(qc), 
    $model as element(model),
    $limitDefinitions as element(WiseSoeQc),
    $dataRows as element(dataRow)*
...
as element(div)
{
    let $limitsList := vldvallim:get-limits($limitDefinitions, "WISE-SoE_WaterQuantity", "WaterReturns")
    ...
    let $infoColumnUrl := "http://dd.eionet.europa.eu/dataelements/84134"
    let $columnsToDisplay := $model/columns/column
